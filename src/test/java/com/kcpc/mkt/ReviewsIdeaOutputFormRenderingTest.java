 ...
 * row per Output Type (Story/Post/Reel/Long Video, in that enum order; PHOTOGRAPHY/VIDEO retired)
 * instead of the old arbitrary Add/Edit/Delete staging table. Only two columns - Output Type and
...
class ReviewsIdeaOutputFormRenderingTest {
    ...
    void plannedOutputsGridRendersOneRowPerOutputTypeWithOnlyTwoColumns() throws Exception {
        ...
        // Photography/Video are fully retired - one row per Output Type, in enum order (STORY,
        // POST, REEL, LONG_VIDEO), every one starting unchecked/disabled.
