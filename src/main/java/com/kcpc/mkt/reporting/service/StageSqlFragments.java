...
/**
 * Shared native-SQL CASE fragments for "which of the 4 governed stages (Shoot/Edit/Publishing/
 * Performance) is this Content Plan currently in, and what is its current approved target date for
 * that stage" - BR-039's "Current Approved Planned Date is stage-context, not a fixed single
 * column." Extracted here (previously duplicated verbatim in both {@link KpiService} and
 * {@link AdminReportingService}) so the KPI Dashboard, the 30-KPI console, and Delayed
...
final class StageSqlFragments {
    ...
    static final String STAGE_PLANNED_DATE_CASE =
            "CASE WHEN wi.current_status_code IN ('SA','SIP','SRV') THEN cp.planned_shoot_date "
    ...
    static final String STAGE_LABEL_CASE =
            "CASE WHEN wi.current_status_code IN ('SA','SIP','SRV','SAP') THEN 'Shoot' "
    ...
    /** Current-stage assignee(s) only - Shoot/Edit's active assignment(s); Publishing/Performance
     * have no single-owner assignment table, resolves to NULL. */
