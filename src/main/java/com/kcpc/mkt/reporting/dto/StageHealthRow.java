 ...
 * or 100) OR when this stage's delay rule is not governed (see {@code delayed} below - currently
 * only Planning). Historical delivery compliance is represented separately (On-Time Delivery /
 * cycle time), not by this column.
 * <p>{@code delayed} is a nullable {@code Long}, not a primitive: {@code null} means "this stage's
 * delay rule is not governed" (see {@code StageDelayPolicy} - currently Planning; renders as "-",
 * never a fabricated 0), distinct from a real, counted {@code 0}.
 */
